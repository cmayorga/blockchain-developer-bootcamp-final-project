...
library SafeERC20 {
    ...
    function callOptionalReturn(IERC20 token, bytes memory data) private {
        ...
        require(success, "SafeERC20: low-level call failed");  //TBD This need to be checked before mainnet
...
contract CONSRewards is LPTokenWrapper, Ownable {
    ...
    function getReward() public updateReward(msg.sender) checkStart checkNextEpoch{
        ...
        if (reward > 0) {
            ...
            CARLOS.transfer(msg.sender, reward); //TBD at mainner should to use SafeERC20, previous line
    ...
    modifier checkNextEpoch() {       
        ...
        if (block.timestamp >= periodFinish && !rewardSystemFinished) {
            ...
            if (fixedCurrentEpochReward > 1) {                
                ...
                CARLOS.mint(address(this), fixedCurrentEpochReward);
    ...
    function notifyRewardAmount(uint256 reward) external onlyOwner updateReward(address(0)) {
        ...
        lastUpdateTime = block.timestamp;
        periodFinish = block.timestamp.add(DURATION);
        CARLOS.mint(address(this), currentEpochReward);
        emit RewardAdded(currentEpochReward);
    }
